...
export const GuideEditorSidebar = () => {
  ...
  const handleGenerateGuide: SubmitHandler<KeyGenerationInput> = async (
    ...
    try {
      const result = await serverGenerateGuideFn({
        data,
      });

      if (!result || !result.graph) {
        throw new Error("No data returned from guide generator.");
      }
  ...
  return (
    <Card asChild>
      <aside className="guide-editor-sidebar">
        <form
          className="guide-editor-sidebar__form"
          onSubmit={handleSubmit(handleGenerateGuide)}
        >
          <section>
            <Heading size="3" mb="4">
              Guide Editor
            ...
            <Box mb="3">
              <Flex justify="between" align="baseline" mb="1">
                <Label.Root htmlFor="options.keyShape">Branch Shape</Label.Root>
                <ConditionalAlert
